...
var Options = {//コマブロに必須とする情報のリスト
    "signal": [
        "hi03ATS_Ps_Directory1", "hi03ATS_Ps_Directory2", "hi03ATS_Ps_Long", "hi03ATS_Ps_Pattern1", "hi03ATS_Ps_Pattern2",
        "hi03ATS_Ps_Pattern3", "hi03ATS_Ps_PatternCallOn", "hi03ATS_Ps_PatternShunting", "hi03ATS_Ps_Prevention"
    ],
    "speed": ["hi03ATS_Ps_SpeedLimitBranch", "hi03ATS_Ps_SpeedLimitEnter"],
    "branchPos": ["hi03ATS_Ps_SpeedLimitBranch"],
    "dir": ["hi03ATS_Ps_SpeedLimitBranch"],
    "time": ["hi03ATS_Ps_PreventionTimer"]
}
...
function onUpdate(entity, scriptExecuter) {
    ...
    if (modelName === "hi03ATS_Ps_SpeedLimitExit") settingsCMDBlock = "{}";
    ...
    if (cmdCache !== settingsCMDBlock) {
        ...
        try {
            ...
            settings = null;
            NGTLog.sendChatMessageToAll("[ATS-Ps] ERROR -> " + e);
        }
    }
    if (!settings) return;
    ...
    if (collideTrain) {
        ...
        if (lastTrainEntity !== collideTrain) {//検知
            ...
            if (Options.signal.indexOf(modelName) !== -1) {
                if (settings.signal === undefined) {
                    NGTLog.sendChatMessageToAll("[ATS-Ps] Can't find signal: " + settings.signal.join(","));
                    return;
                }
            ...
            if (modelName === "hi03ATS_Ps_SpeedLimitBranch" && settings.signal !== undefined) {
            ...
            //NGTLog.sendChatMessageToAll("[ATS-Ps debug] SendData:" + sendDataJson.replace(/☆/g, ","));

            //ATSプラグイン未搭載車への対応(ATS-Sn機能)
            if (trainDataMap.getString("ATSType") === "" && signalLevel === 1) {
                ...
                //ロング地上子
                if (modelName === "hi03ATS_Ps_Long") {
                    if (core) core.setSignal(1);
                }

                //直下地上子
                if (modelName === "hi03ATS_Ps_Directory1" || modelName === "hi03ATS_Ps_Directory2") {
                    if (core) core.setSignal(1);
        ...
        if (modelName === "hi03ATS_Ps_Long" || modelName === "hi03ATS_Ps_Directory1" || modelName === "hi03ATS_Ps_Directory2") {
            var rail = getTileEntity(world, x, y, z);
            if (!(rail instanceof TileEntityLargeRailBase)) rail = getTileEntity(world, x, y - 1, z);
...
function getSendData(world, modelName, signalLevel, limitSpeed, branchPos, branchDir, time, distance, limitLength) {
    ...
    //誤出発防止ﾀｲﾏｰ起動地上子
    else if (modelName === "hi03ATS_Ps_PreventionTimer") {
        sendData.time = Math.floor(time * 20);
    }

    //分岐制限地上子
    else if (modelName === "hi03ATS_Ps_SpeedLimitBranch") {
        if (!branchPos) {
            NGTLog.sendChatMessageToAll('[ATS-Ps] Not find "branchPos"');
        }
        else if (signalLevel !== 1) {
            var block = getBlock(world, branchPos[0], branchPos[1], branchPos[2]);
            if ((branchDir && block === Blocks.RedstoneBlock) || (!branchDir && block !== Blocks.RedstoneBlock)) {
                sendData.pattern3 = limitSpeed;
                sendData.distance3 = distance ? distance : 555;
                sendData.limitLength = limitLength ? limitLength : 50;
            }
        }
    }

    //速度制限地上子
    else if (modelName === "hi03ATS_Ps_SpeedLimitEnter") {
        sendData.pattern4 = limitSpeed;
        sendData.distance4 = distance ? distance : 555;
    }

    //速度制限解除地上子
    else if (modelName === "hi03ATS_Ps_SpeedLimitExit") {
        sendData.pattern4 = null;
        sendData.distance4 = null;
    }

    sendData.atsType = "ATS-Ps";
    return sendData;
}
