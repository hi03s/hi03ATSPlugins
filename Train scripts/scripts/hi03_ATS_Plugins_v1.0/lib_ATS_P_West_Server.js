...
function operationATS_P_West(entity, currentATSType, options) {
    ...
    var _fx = operationATS_P_West._fx;
    if (!_fx) {
        _fx = {};
        ...
        operationATS_P_West._fx = _fx;
    }

    //#####################
    //###   ATS理論部   ###
    //#####################
    var _ats = operationATS_P_West._ats;
    if (!_ats) {
        _ats = {};
        ...
        _ats.reset = function (entity, options) {
            ...
            //-- ATS-Sx機能 --
            _fx.setBooleanSync(dataMap, "ATS-P_West_isLongAlertLatched", true);//キンコン音
        ...
        _ats.off = function (entity, currentATSType, options) {
            ...
            //-- ATS-Sx機能 --
            _fx.setBooleanSync(dataMap, "ATS-P_West_isATSBrake", false);
            _fx.setBooleanSync(dataMap, "ATS-P_West_isATSLongBrake", false);
            _fx.setBooleanSync(dataMap, "ATS-P_West_isLongAlert", false);
            _fx.setBooleanSync(dataMap, "ATS-P_West_isAtsFault", false);
            _fx.setBooleanSync(dataMap, "ATS-P_West_isATSBrakeDisable", false);
            _fx.setBooleanSync(dataMap, "ATS-P_West_acknowledgeAlert", false);
            ...
            _fx.setBooleanSync(dataMap, "ATS-P_West_isRollbackBrake", false);
            _fx.setBooleanSync(dataMap, "ATS-P_West_isATSPBrake", false);
        };
        ...
        _ats.receiveData = function (entity, atsState, receiveData, options) {
            ...
            var isActiveATSP = dataMap.getBoolean("ATS-P_West_isActiveATSP");
            var ATSPMode = dataMap.getString("ATS-P_West_ATSPMode");
            ...
            if ((ATSPMode === "East" && !isActiveATSP) || ATSPMode === "West") {//東モード:ATS-P動作時はSx無効、西モード:ATS-P・Sx併用
                //即時停止
                if (receiveData.stop1 === true && !isATSBrakeDisable) _fx.setBooleanSync(dataMap, "ATS-P_West_isATSBrake", true);

                //即時停止(誤出発防止地上子用)
                if (receiveData.stop2 === true && atsState.preventionTimerTick === 0 && !isATSBrakeDisable) _fx.setBooleanSync(dataMap, "ATS-P_West_isATSBrake", true);
            ...
            if (isActiveATSP) {
                //即時停止
                if (receiveData.stop === true && !isATSBrakeDisable) _fx.setBooleanSync(dataMap, "ATS-P_West_isATSPBrake", true);
    ...
    _ats.operation = function (entity, atsState, options) {
        ...
        //-- ATS-Sx機能 --
        //ATSブレーキ開放
        var pushBrakeDisableButton = dataMap.getBoolean("ATS-P_West_pushBrakeDisableButton");//ATSブレーキ開放ボタン
        if (pushBrakeDisableButton) atsState.disableCount = 1200;
        var isATSBrakeDisable = atsState.disableCount > 0;
        _fx.setBooleanSync(dataMap, "ATS-P_West_isATSBrakeDisable", isATSBrakeDisable);
        ...
        //ATSロング警報
        var isAcknowledgeAlert = dataMap.getBoolean("ATS-P_West_acknowledgeAlert");
        if (isAcknowledgeAlert) atsState.alertCount = -1;//確認押下で解除
        else if (atsState.alertCount === 0) {
            _fx.setBooleanSync(dataMap, "ATS-P_West_isATSLongBrake", true);
        ...
        if (speed <= -7 && entity.getTrainStateData(10) === 0) {
            _fx.setBooleanSync(dataMap, "ATS-P_West_isRollbackBrake", true);
        }
        ...
        _fx.setBooleanSync(dataMap, "ATS-P_West_patternAlert", (speed2 > patternAlertSpeed2));

        return atsState;//最後にatsStateを返す
    };

    operationATS_P_West._ats = _ats;
