...
//##                             ##
//#################################
var ATS_Ps_State = function (options) {
    ...
    this.KeyMap = options.KeyMap || {
        ...
        switchButton_ps: this.Keyboard.KEY_1,
...
ATS_Ps_Display.prototype = {
    ...
    onUpdate: function (entity, pass) {
        ...
        var atsSwitchButton1 = this.Keyboard.isKeyDown(this.KeyMap.switchButton_ps);
        var atsSwitchButton2 = this.Keyboard.isKeyDown(this.KeyMap.switchButton_off);
        if (dataMap.getBoolean("ATS-Ps_isInitialize")) return;

        if (isAutoATS) {
            //ATS確認ボタン(自動)
            var world = entity.field_70170_p;
            var worldTick = world.func_82737_E();
            var longAlertTick = dataMap.getInt("ATS-Ps_longAlertTick");
            var isATSAlert = dataMap.getBoolean("ATS-Ps_isLongAlert");
            var isLongAlert = dataMap.getBoolean("ATS-Ps_isLongAlertLatched");
            ...
            if (!optionKey && !atsButton && (speed <= 0) && notch <= this.MaxBrakeNotch && (isATSBrake || isATSLongBrake || isRollbackBrake || isPatternBrake)) disableButton = true;
        }

        //ATS確認ボタン
        this._setBooleanSync(dataMap, "ATS-Ps_acknowledgeAlert", atsButton && notch < 0);

        //ATS警報持続ボタン
        if (optionKey && atsButton) this._setBooleanSync(dataMap, "ATS-Ps_isLongAlertLatched", false);
        this._setBooleanSync(dataMap, "ATS-Ps_pushAlertButton", optionKey && atsButton);

        //ATS復帰ボタン
        if (disableButton && (notch <= this.MaxBrakeNotch)) {
            this._setBooleanSync(dataMap, "ATS-Ps_isATSBrake", false);
            this._setBooleanSync(dataMap, "ATS-Ps_isATSLongBrake", false);
            this._setBooleanSync(dataMap, "ATS-Ps_isRollbackBrake", false);
            this._setBooleanSync(dataMap, "ATS-Ps_isPatternBrake", false);
        }

        //ブレーキ開放ボタン
        this._setBooleanSync(dataMap, "ATS-Ps_pushBrakeDisableButton", optionKey && disableButton);
