...
//##                                 ##
//#####################################
var ATS_P_West_State = function (options) {
    ...
    this.KeyMap = options.KeyMap || {
        ...
        switchButton_p_west: this.Keyboard.KEY_1,
...
ATS_P_West_Display.prototype = {
    ...
    onUpdate: function (entity, pass) {
        ...
        var atsSwitchButton1 = this.Keyboard.isKeyDown(this.KeyMap.switchButton_p_west);
        var atsSwitchButton2 = this.Keyboard.isKeyDown(this.KeyMap.switchButton_off);
        if (dataMap.getBoolean("ATS-P_West_isInitialize")) return;

        if (isAutoATS) {
            //ATS確認ボタン(自動)
            var world = entity.field_70170_p;
            var worldTick = world.func_82737_E();
            var longAlertTick = dataMap.getInt("ATS-P_West_longAlertTick");
            var isATSAlert = dataMap.getBoolean("ATS-P_West_isLongAlert");
            var isLongAlert = dataMap.getBoolean("ATS-P_West_isLongAlertLatched");
            ...
            //ATS復帰ボタン(自動)
            var isATSBrake = dataMap.getBoolean("ATS-P_West_isATSBrake");
            var isATSPBrake = dataMap.getBoolean("ATS-P_West_isATSPBrake");
            var isATSLongBrake = dataMap.getBoolean("ATS-P_West_isATSLongBrake");
            var isPatternBrake = dataMap.getBoolean("ATS-P_West_isPatternBrake");
            var isPatternBrakeFull = dataMap.getBoolean("ATS-P_West_isPatternBrakeFull");
            var isRollbackBrake = dataMap.getBoolean("ATS-P_West_isRollbackBrake");
        ...
        //ATS確認ボタン
        this._setBooleanSync(dataMap, "ATS-P_West_acknowledgeAlert", atsButton && notch < 0);

        //ATS警報持続ボタン
        if (optionKey && atsButton) this._setBooleanSync(dataMap, "ATS-P_West_isLongAlertLatched", false);
        this._setBooleanSync(dataMap, "ATS-P_West_pushAlertButton", optionKey && atsButton);

        //ATS復帰ボタン
        if (disableButton && (notch <= this.MaxBrakeNotch)) {
            this._setBooleanSync(dataMap, "ATS-P_West_isATSBrake", false);
            this._setBooleanSync(dataMap, "ATS-P_West_isATSPBrake", false);
            this._setBooleanSync(dataMap, "ATS-P_West_isATSLongBrake", false);
            this._setBooleanSync(dataMap, "ATS-P_West_isPatternBrake", false);
            this._setBooleanSync(dataMap, "ATS-P_West_isPatternBrakeFull", false);
            this._setBooleanSync(dataMap, "ATS-P_West_isRollbackBrake", false);
        }

        //ブレーキ開放ボタン
        this._setBooleanSync(dataMap, "ATS-P_West_pushBrakeDisableButton", optionKey && disableButton);
    ...
    getState: function (entity) {
        ...
        return {
            isATSBrake: dm.getBoolean("ATS-P_West_isATSBrake"),                 //ATSブレーキ:直下地上子
            isPatternBrake: dm.getBoolean("ATS-P_West_isPatternBrake"),         //ATSブレーキ:パターン抵触
            ...
            isBrakeDisable: dm.getBoolean("ATS-P_West_isATSBrakeDisable"),      //ブレーキ開放
            isLongAlertLatched: dm.getBoolean("ATS-P_West_isLongAlertLatched"), //ATS警報持続(キンコン)
            isAlertButton1Pressed: dm.getBoolean("ATS-P_West_acknowledgeAlert"),//ATS確認ボタン押下
            isAlertButton2Pressed: dm.getBoolean("ATS-P_West_pushAlertButton"), //ATS警報持続ボタン押下
            isInitialize: dm.getBoolean("ATS-P_West_isInitialize"),             //ATS初期化中
            isActiveATSP: dm.getBoolean("ATS-P_West_isActiveATSP"),             //ATS-P有効化
            isATSPBrake: dm.getBoolean("ATS-P_West_isATSPBrake"),               //ATS-P非常ブレーキ(直下地上子)
            atspMode: dm.getString("ATS-P_West_ATSPMode")                       //ATS-Pモード(東:"East", 西:"West")
        }
    },
